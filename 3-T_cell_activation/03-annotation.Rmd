...
To annotate the cell types, we are going to analyze each day and donor separately, as we need as much resolution as possible to detect the markers.
...
Finally, as we stimulated the PBMC with anti-CD3 antibody, we aim to find which cells exhibit an activated T cell signature. Thus, we will manually curate the upregulated genes from [Garcia-Sousa I, et al.](https://bmcgenomics.biomedcentral.com/articles/10.1186/s12864-019-5967-8#Sec20), where they profiled the gene expression changes upon human T cell stimulation with anti-CD3 antibodies.
...
NOTE: 
- Comment on AIF1 , GZMA, GZMB, GZMH as markers of activated T cells
- Comment on: anti CD3 signature also detects active cells in non-active datasets (cytotoxic cells). Most upregulated GO term in the anti CD3 paper is cell cycle, comment on the fact that we use cell cycle as a proxy to detect active T cells.
