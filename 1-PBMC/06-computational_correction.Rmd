---
title: "Computational Correction Cold Shock"
...
The objective of this notebook is to correct the cold-shock-biased transcriptomes of the PBMC is our dataset using the signature we obtained from the CLL diseased donors. Thus, the PBMC will serve as test set to elucidate the usability of this correction with real data. We will aim to remove the technical variability while preserving the biological
...
# Regress Out
Now, let us regress the expression of each gene on cold shock score. We will do this for each cell type separately to minimize [Simpson's paradox](https://ds.czbiohub.org/blog/Regression-Hazards/). Then, we will keep the residuals as the variability not explained by time:
